...
G_DEFINE_TYPE_WITH_PRIVATE(XAppDarkModeManager, xapp_dark_mode_manager,
                           G_TYPE_OBJECT)

static const gchar *color_scheme_name(ColorScheme scheme) {
  switch (scheme) {
...
static void update_gtk_settings(XAppDarkModeManager *manager) {
  XAppDarkModeManagerPrivate *priv =
      xapp_dark_mode_manager_get_instance_private(manager);
...
static void fallback_gsettings_changed_cb(gpointer user_data) {
  XAppDarkModeManager *manager = XAPP_DARK_MODE_MANAGER(user_data);
  XAppDarkModeManagerPrivate *priv =
      xapp_dark_mode_manager_get_instance_private(manager);

  priv->color_scheme =
      g_settings_get_enum(priv->fallback_settings, "color-scheme");

  DEBUG("Fallback settings changed (color-scheme: %s)",
        color_scheme_name(priv->color_scheme));
  update_gtk_settings(manager);
}

#define GNOME_SETTINGS "org.gnome.desktop.interface"

static void use_fallback_gsettings(XAppDarkModeManager *manager) {
  XAppDarkModeManagerPrivate *priv =
      xapp_dark_mode_manager_get_instance_private(manager);
  ...
  if (g_settings_schema_source_lookup(g_settings_schema_source_get_default(),
                                      FALLBACK_SETTINGS, TRUE) != NULL) {
    priv->fallback_settings = g_settings_new(FALLBACK_SETTINGS);
  } else if (g_settings_schema_source_lookup(
                 g_settings_schema_source_get_default(), GNOME_SETTINGS,
                 TRUE) != NULL) {
    priv->fallback_settings = g_settings_new(GNOME_SETTINGS);
  }

  if (priv->fallback_settings != NULL) {
    priv->color_scheme =
        g_settings_get_enum(priv->fallback_settings, "color-scheme");

    g_signal_connect_swapped(priv->fallback_settings, "changed::color-scheme",
                             G_CALLBACK(fallback_gsettings_changed_cb),
                             manager);

    GSettingsSchema *schema = NULL;
    g_object_get(priv->fallback_settings, "settings-schema", &schema, NULL);

    if (schema != NULL) {
      const gchar *schema_id = g_settings_schema_get_id(schema);

      DEBUG("Initial fallback settings read from %s (color-scheme: %s)",
            schema_id, color_scheme_name(priv->color_scheme));
      g_settings_schema_unref(schema);
    }
  } else {
    DEBUG("No fallback schema found");
  }

  update_gtk_settings(manager);
}

static gboolean read_portal_setting(XAppDarkModeManager *manager,
                                    const char *schema, const char *name,
                                    const char *type, GVariant **out) {
  XAppDarkModeManagerPrivate *priv =
      xapp_dark_mode_manager_get_instance_private(manager);
  ...
  ret = g_dbus_proxy_call_sync(
      priv->portal, "Read", g_variant_new("(ss)", schema, name),
      G_DBUS_CALL_FLAGS_NONE, 1000, /* 1 second timeout instead of infinite */
      NULL, &error);
  if (error) {
  ...
  if (g_variant_type_equal(g_variant_get_type(child2), out_type)) {
    *out = child2;
    ...
    g_critical("Invalid type for %s.%s: expected %s, got %s", schema, name,
               type, g_variant_get_type_string(child2));
...
static void portal_changed_cb(GDBusProxy *proxy, const char *sender_name,
  ...
  DEBUG("Portal setting changed (color-scheme: %s)",
        color_scheme_name(priv->color_scheme));

  update_gtk_settings(manager);
}

static void init_and_monitor_portal(XAppDarkModeManager *manager) {
  XAppDarkModeManagerPrivate *priv =
      xapp_dark_mode_manager_get_instance_private(manager);
  GVariant *value = NULL;

  if (read_portal_setting(manager, "org.freedesktop.appearance", "color-scheme",
                          "u", &value)) {
    priv->color_scheme = g_variant_get_uint32(value);
    g_variant_unref(value);

    DEBUG("Initial portal setting read (color-scheme: %s)",
          color_scheme_name(priv->color_scheme));

    update_gtk_settings(manager);

    g_signal_connect(priv->portal, "g-signal", G_CALLBACK(portal_changed_cb),
                     manager);
  } else {
...
static void new_portal_callback(GObject *source, GAsyncResult *res,
                                gpointer user_data) {
  ...
  if (error != NULL) {
    g_critical("XDG desktop portal proxy failed to initialize: %s",
               error->message);
    g_free(error);
    use_fallback_gsettings(manager);
    return;
...
static void init_manager(XAppDarkModeManager *manager) {
  ...
  g_dbus_proxy_new_for_bus(G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, NULL,
                           ...
                           PORTAL_SETTINGS_INTERFACE, NULL,
                           (GAsyncReadyCallback)new_portal_callback, manager);
}

static void xapp_dark_mode_manager_init(XAppDarkModeManager *manager) {}

static void xapp_dark_mode_manager_dispose(GObject *object) {
...
static void xapp_dark_mode_manager_finalize(GObject *object) {
  DEBUG("XAppDarkModeManager finalize (%p)", object);

  G_OBJECT_CLASS(xapp_dark_mode_manager_parent_class)->finalize(object);
}

static void xapp_dark_mode_manager_class_init(XAppDarkModeManagerClass *klass) {
 ...
 *
 *
 * Returns: (transfer full): a new XAppDarkModeManager instance. Free with
 * g_object_unref.
 *
 * Since: 2.6
 */
XAppDarkModeManager *xapp_dark_mode_manager_new(gboolean prefer_dark_mode) {
  XAppDarkModeManager *manager =
      g_object_new(XAPP_TYPE_DARK_MODE_MANAGER, NULL);
  XAppDarkModeManagerPrivate *priv =
      xapp_dark_mode_manager_get_instance_private(manager);
