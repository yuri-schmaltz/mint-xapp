...
#define SYNC_METADATA_KEY "sync-file-metadata"
#define SETTINGS_DELIMITER "::"
#define MAX_DISPLAY_URI_LENGTH 20

G_DEFINE_BOXED_TYPE(XAppFavoriteInfo, xapp_favorite_info,
                    xapp_favorite_info_copy, xapp_favorite_info_free);
/**
 ...
 *
 * A list of #XAppFavoriteInfos can be retrieved in full, or only for specific
 * mimetypes.
 *
 * A favorites uri scheme is also available if the xapp gtk3 module is loaded
 * and this also makes the uri available as a shortcut in file dialogs.
 *
 ...
 *
 * Returns: (transfer full): a new #XAppFavoriteInfo.  Free using
...
static void finish_add_favorite(XAppFavorites *favorites, const gchar *uri,
                                const gchar *mimetype, gboolean from_saved);

static gboolean changed_callback(gpointer data) {
...
static void queue_changed(XAppFavorites *favorites) {
  XAppFavoritesPrivate *priv = xapp_favorites_get_instance_private(favorites);
...
static void sync_metadata_callback(GObject *source, GAsyncResult *res,
                                   gpointer user_data) {
  GFile *file;
  GError *error = NULL;
  ...
  if (!g_file_set_attributes_finish(file, res, NULL, &error)) {
    if (error != NULL) {
      if (error->code != G_IO_ERROR_NOT_FOUND) {
        g_warning("Could not update file metadata for favorite file '%s': %s",
...
static void sync_file_metadata(XAppFavorites *favorites, const gchar *uri,
  ...
  if (priv->settings == NULL ||
      !g_settings_get_boolean(priv->settings, SYNC_METADATA_KEY)) {
    return;
  }

  /* We only sync metadata for local files */
  if (!g_str_has_prefix(uri, "file://") && !g_str_has_prefix(uri, "/")) {
    return;
  }
  ...
  if (is_favorite) {
    ...
    /* Unset the key */
    g_file_info_set_attribute(info, FAVORITE_METADATA_KEY,
  ...
  g_file_set_attributes_async(file, info, G_FILE_QUERY_INFO_NONE,
                              G_PRIORITY_DEFAULT, NULL, sync_metadata_callback,
                              favorites);
...
static void store_favorites(XAppFavorites *favorites) {
  XAppFavoritesPrivate *priv = xapp_favorites_get_instance_private(favorites);
  GList *iter, *keys;
  GPtrArray *array;
  gchar **new_settings;
  ...
  for (iter = keys; iter != NULL; iter = iter->next) {
    XAppFavoriteInfo *info =
        (XAppFavoriteInfo *)g_hash_table_lookup(priv->infos, iter->data);
    gchar *entry;

    entry =
        g_strjoin(SETTINGS_DELIMITER, info->uri, info->cached_mimetype, NULL);
  ...
  g_settings_set_strv(priv->settings, FAVORITES_KEY,
                      (const gchar *const *)new_settings);
...
static void load_favorites(XAppFavorites *favorites, gboolean signal_changed) {
  XAppFavoritesPrivate *priv = xapp_favorites_get_instance_private(favorites);
  gchar **raw_list;
  gint i;
  ...
  priv->infos = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
...
static void rename_favorite(XAppFavorites *favorites, const gchar *old_uri,
                            const gchar *new_uri) {
  XAppFavoriteInfo *info;
  XAppFavoritesPrivate *priv = xapp_favorites_get_instance_private(favorites);
  ...
  if (g_str_has_prefix(old_uri, ROOT_URI)) {
    ...
    if (info) {
      ...
      renamed_file =
          g_file_get_child_for_display_name(parent, new_display_name, NULL);
  ...
  if (info != NULL && final_new_uri != NULL) {
    ...
    g_hash_table_remove(priv->infos, (gconstpointer)info->uri);

    finish_add_favorite(favorites, final_new_uri, mimetype, FALSE);
...
static void remove_favorite(XAppFavorites *favorites, const gchar *uri) {
  XAppFavoritesPrivate *priv = xapp_favorites_get_instance_private(favorites);
  gchar *real_uri;
  ...
  // It may be orphaned for some reason.. even if it's not in gsettings, still
  // try to remove the favorite attribute.
  sync_file_metadata(favorites, real_uri, FALSE);

  if (!g_hash_table_remove(priv->infos, real_uri)) {
    DEBUG(
        "XAppFavorites: remove_favorite: could not find favorite for uri '%s'",
        real_uri);
    g_free(real_uri);
    return;
  }
...
static void deduplicate_display_names(XAppFavorites *favorites,
                                      GHashTable *infos) {
  GList *fav_uris, *ptr;
  GHashTable *lists_of_keys_by_basename =
  ...
  for (ptr = fav_uris; ptr != NULL; ptr = ptr->next) {
    ...
    uris =
        g_hash_table_lookup(lists_of_keys_by_basename, original_display_name);
    uris = g_list_prepend(uris, g_strdup(uri));

    g_hash_table_replace(lists_of_keys_by_basename,
                         g_strdup(original_display_name), uris);
  ...
  while (g_hash_table_iter_next(&iter, &key, &value)) {
    ...
    // Now we know we have a list of uris that would have identical display
    // names Add a part of the uri after each to distinguish them.
    ...
    for (uri_ptr = same_names_list; uri_ptr != NULL; uri_ptr = uri_ptr->next) {
      ...
      // {
      //     gchar *home_rpath = g_file_get_relative_path (home_file,
      //     parent_file); gchar *home_basename = g_file_get_basename
      //     (home_file);
...
static void on_display_name_received(GObject *source, GAsyncResult *res,
                                     gpointer user_data) {
  GFile *file;
  GFileInfo *file_info;
  GError *error;
  ...
  if (error) {
    DEBUG("XAppFavorites: problem trying to get real display name for uri "
          "'%s': %s",
          uri, error->message);
    g_error_free(error);
    return;
...
static void finish_add_favorite(XAppFavorites *favorites, const gchar *uri,
  ...
  // Check if it's there again, in case it was added while we were getting
  // mimetype.
  ...
  g_file_query_info_async(gfile, G_FILE_ATTRIBUTE_STANDARD_DISPLAY_NAME,
                          G_FILE_QUERY_INFO_NONE, G_PRIORITY_LOW, NULL,
                          on_display_name_received, favorites);
...
static void on_content_type_info_received(GObject *source, GAsyncResult *res,
                                          gpointer user_data) {
  XAppFavorites *favorites = XAPP_FAVORITES(user_data);
  GFile *file;
  GFileInfo *file_info;
  ...
  if (error) {
    DEBUG("XAppFavorites: problem trying to figure out content type for uri "
          "'%s': %s",
          uri, error->message);
    g_error_free(error);
  }
  ...
  if (file_info) {
    cached_mimetype = g_strdup(g_file_info_get_attribute_string(
    ...
    finish_add_favorite(favorites, uri, cached_mimetype, FALSE);
...
static void add_favorite(XAppFavorites *favorites, const gchar *uri) {
  XAppFavoritesPrivate *priv = xapp_favorites_get_instance_private(favorites);
  GFile *file;
  ...
  g_file_query_info_async(file, G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE,
                          G_FILE_QUERY_INFO_NONE, G_PRIORITY_LOW, NULL,
                          on_content_type_info_received, favorites);
}

static void on_settings_list_changed(GSettings *settings, gchar *key,
                                     gpointer user_data) {
  XAppFavorites *favorites = XAPP_FAVORITES(user_data);
...
static void xapp_favorites_init(XAppFavorites *favorites) {
  XAppFavoritesPrivate *priv = xapp_favorites_get_instance_private(favorites);

  DEBUG("XAppFavorites: init:");

  if (g_settings_schema_source_lookup(g_settings_schema_source_get_default(),
                                      FAVORITES_SCHEMA, TRUE) == NULL) {
    g_warning(
        "XAppFavorites: schema %s not found. Favorites will not be persisted.",
        FAVORITES_SCHEMA);
    return;
  }

  priv->settings = g_settings_new(FAVORITES_SCHEMA);
  priv->settings_listener_id =
      g_signal_connect(priv->settings, "changed::" FAVORITES_KEY,
                       G_CALLBACK(on_settings_list_changed), favorites);

  load_favorites(favorites, FALSE);
}

static void xapp_favorites_dispose(GObject *object) {
...
static void xapp_favorites_finalize(GObject *object) {
  DEBUG("XAppFavorites finalize (%p)", object);

  G_OBJECT_CLASS(xapp_favorites_parent_class)->finalize(object);
}

static void xapp_favorites_class_init(XAppFavoritesClass *klass) {
   ...
   */
  signals[CHANGED] = g_signal_new("changed", XAPP_TYPE_FAVORITES,
 ...
 *
 * Returns: (transfer none): the XAppFavorites instance for the process. Do not
 * free.
 *
 * Since: 2.0
 */
...
void match_mimetypes(gpointer key, gpointer value, gpointer user_data) {
  ...
  for (i = 0; i < g_strv_length((gchar **)data->mimetypes); i++) {
    if (g_content_type_is_mime_type(info->cached_mimetype,
                                    data->mimetypes[i])) {
      data->items = g_list_prepend(data->items, xapp_favorite_info_copy(info));
      return;
    }
 ...
 * @mimetypes: (nullable) (array zero-terminated=1): The mimetypes to filter by
 for results
 *
 * Gets a list of all favorites.  If mimetype is not %NULL, the list will
 * contain only favorites with that mimetype.
 *
 * Returns: (element-type XAppFavoriteInfo) (transfer full): a list of
 #XAppFavoriteInfos. Free the list with #g_list_free, free elements with
 #xapp_favorite_info_free.
 *
 * Since: 2.0
 */
GList *xapp_favorites_get_favorites(XAppFavorites *favorites,
  ...
  g_hash_table_foreach(priv->infos, (GHFunc)match_mimetypes, &data);

  ret = g_list_reverse(data.items);

  gchar *typestring = mimetypes ? g_strjoinv(", ", (gchar **)mimetypes) : NULL;
  DEBUG("XAppFavorites: get_favorites returning list for mimetype '%s' (%d "
        "items)",
...
static gboolean lookup_display_name(gpointer key, gpointer value,
                                    gpointer user_data) {
 ...
 *
 * Returns: (transfer none): an XAppFavoriteInfo or NULL if one was not found.
 * This is owned by the favorites manager and should not be freed.
 *
 * Since: 2.0
 */
XAppFavoriteInfo *
xapp_favorites_find_by_display_name(XAppFavorites *favorites,
  ...
  info = g_hash_table_find(priv->infos, (GHRFunc)lookup_display_name,
...
static void launch_uri_callback(GObject *source, GAsyncResult *res,
                                gpointer user_data) {
  ...
  if (!g_app_info_launch_default_for_uri_finish(res, &error)) {
    if (error) {
      DEBUG("XAppFavorites: launch: error opening uri '%s': %s", uri,
            error->message);
      g_error_free(error);
    }
  }
 ...
 * Since: 2.0
 */
void xapp_favorites_launch(XAppFavorites *favorites, const gchar *uri,
                           guint32 timestamp) {
  GdkDisplay *display;
  ...
  g_app_info_launch_default_for_uri_async(
      uri, G_APP_LAUNCH_CONTEXT(launch_context), NULL, launch_uri_callback,
      g_strdup(uri));
 ...
 * a convenience instead of add/remove, and guarantees the result, without
 * having to worry about multiple dbus calls (gsettings).
 *
 * Since: 2.0
 */
void xapp_favorites_rename(XAppFavorites *favorites, const gchar *old_uri,
                           const gchar *new_uri) {
  g_return_if_fail(XAPP_IS_FAVORITES(favorites));
...
static void cb_data_destroy_notify(gpointer callback_data, GObject *object) {
...
static void free_item_callback_data(gpointer callback_data, GClosure *closure) {
  ItemCallbackData *data = (ItemCallbackData *)callback_data;
  g_free(data->uri);
  g_slice_free(ItemCallbackData, data);
}

static void item_activated(GObject *item, gpointer user_data) {
  ItemCallbackData *data = (ItemCallbackData *)user_data;

  data->callback(data->favorites, data->uri, data->user_data);
}

static void remove_menu_item(GtkWidget *item, gpointer user_data) {
  gtk_container_remove(GTK_CONTAINER(user_data), item);
}

static void populate_menu(XAppFavorites *favorites, GtkMenu *menu) {
  GList *fav_list, *ptr;
  GtkWidget *item;
  XAppFavoritesItemSelectedCallback callback;
  gpointer user_data;
  ...
  gtk_container_foreach(GTK_CONTAINER(menu), (GtkCallback)remove_menu_item,
                        menu);
  ...
  for (ptr = fav_list; ptr != NULL; ptr = ptr->next) {
    ...
    g_signal_connect_data(item, "activate", G_CALLBACK(item_activated), data,
                          (GClosureNotify)free_item_callback_data, 0);
  }
...
static void refresh_menu_items(XAppFavorites *favorites, gpointer user_data) {
  g_return_if_fail(XAPP_IS_FAVORITES(favorites));
 ...
 * @mimetypes: (nullable): The mimetypes to filter for, or NULL to include all
 favorites.
 * @callback: (scope notified): (closure user_data): The callback to use when a
 menu item has been selected.
 * @user_data: The data to pass to the callback.
 * @func: Destroy function for user_data.
 *
 * Generates a GtkMenu widget populated with favorites. The callback will be
 called when
 * a menu item has been activated, and will include the uri of the respective
 item.
 *
 * Returns: (transfer full): a new #GtkMenu populated with a list of favorites,
 or NULL if there are no favorites.
 *
 * Since: 2.0
 */
GtkWidget *
xapp_favorites_create_menu(XAppFavorites *favorites, const gchar **mimetypes,
                           XAppFavoritesItemSelectedCallback callback,
  ...
  g_object_set_data_full(G_OBJECT(menu), "mimetype",
                         g_strdupv((gchar **)mimetypes),
                         (GDestroyNotify)g_strfreev);

  g_object_set_data(G_OBJECT(menu), "activate-cb", callback);

  g_object_set_data(G_OBJECT(menu), "user-data", user_data);
  ...
  dd->update_id = g_signal_connect(favorites, "changed",
                                   G_CALLBACK(refresh_menu_items), menu);
...
static GList *populate_action_list(XAppFavorites *favorites,
                                   const gchar **mimetypes) {
  GList *fav_list, *ptr;
  GList *actions;
  GtkAction *action;
  ...
  for (ptr = fav_list, i = 0; ptr != NULL; ptr = ptr->next, i++) {
    ...
    if (mimetypes != NULL) {
      action = g_object_new(GTK_TYPE_ACTION, "name", info->uri, "label",
      ...
      action = g_object_new(GTK_TYPE_ACTION, "name", info->uri, "label",
                            info->display_name, "gicon", icon, NULL);
 ...
 *
 * Returns: (element-type Gtk.Action) (transfer full): a new #GtkActionGroup
 populated with a list of favorites, or NULL if there are no favorites.
...
GList *xapp_favorites_create_actions(XAppFavorites *favorites,
  ...
  actions = populate_action_list(favorites, mimetypes);
