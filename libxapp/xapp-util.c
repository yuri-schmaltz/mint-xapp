...
gboolean xapp_util_gpu_offload_supported(void) {
  ...
  if (!g_file_load_contents(modefile, NULL, &contents, NULL, NULL, NULL)) {
    return FALSE;
  }
...
gboolean xapp_util_get_session_is_running(void) {
  ...
  if (error != NULL) {
    g_critical("Unable to determine if session is running, could not get "
               "session bus: %s\n",
               error->message);
    g_clear_error(&error);
  ...
  session_result = g_dbus_connection_call_sync(
      session_bus, "org.gnome.SessionManager", "/org/gnome/SessionManager",
      "org.gnome.SessionManager", "IsSessionRunning", NULL,
 ...
 * the css "font" tag. The font description must contain the font family and
 * font size or conversion will fail and %NULL will be returned
 *
...
gchar *xapp_pango_font_string_to_css(const char *pango_font_string) {
  ...
  g_string_append_printf(font_string, "%dpx ",
 ...
 * xapp_get_tmp_dir:
 *
 * Provides the path to the system's temporary files folder. This is identical
 * to g_get_tmp_dir, but includes the /dev/shm ramdisk as the first choice for a
 * temporary folder.
 *
 * Returns: (type filename) (transfer none): the directory to use for temporary
 * files. Since: 2.2.16
 */
