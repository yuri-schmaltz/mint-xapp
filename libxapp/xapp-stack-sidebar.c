...
enum { PROP_0, PROP_STACK, N_PROPERTIES };
...
static void xapp_stack_sidebar_set_property(GObject *object, guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
  switch (prop_id) {
  case PROP_STACK:
    xapp_stack_sidebar_set_stack(XAPP_STACK_SIDEBAR(object),
...
static void xapp_stack_sidebar_get_property(GObject *object, guint prop_id,
...
static gint sort_list(GtkListBoxRow *row1, GtkListBoxRow *row2,
                      gpointer userdata) {
  ...
  if (row1) {
    ...
    gtk_container_child_get(GTK_CONTAINER(sidebar->stack), widget, "position",
                            &left, NULL);
  }
...
static void xapp_stack_sidebar_row_selected(GtkListBox *box, GtkListBoxRow *row,
                                            gpointer user_data) {
...
static void xapp_stack_sidebar_init(XAppStackSidebar *sidebar) {
  ...
  atk_object_set_name(gtk_widget_get_accessible(GTK_WIDGET(sidebar->list)),
                      _("Stack pages"));
...
static void update_row(XAppStackSidebar *sidebar, GtkWidget *widget,
                       GtkWidget *row) {
  GList *children;
  GList *list;
  GtkWidget *item;
  ...
  gtk_container_child_get(GTK_CONTAINER(sidebar->stack), widget, "title",
                          &title, "icon-name", &icon_name, "needs-attention",
  ...
  gtk_widget_set_visible(row, gtk_widget_get_visible(widget) &&
                                  (title != NULL || icon_name != NULL));
...
static void on_position_updated(GtkWidget *widget, GParamSpec *pspec,
                                XAppStackSidebar *sidebar) {
  gtk_list_box_invalidate_sort(sidebar->list);
}

static void on_child_updated(GtkWidget *widget, GParamSpec *pspec,
                             XAppStackSidebar *sidebar) {
  GtkWidget *row;
...
static void add_child(GtkWidget *widget, XAppStackSidebar *sidebar) {
  GtkWidget *item;
  GtkWidget *label;
  GtkWidget *icon;
  GtkWidget *row;
  ...
  /* Hook up events */
  g_signal_connect(widget, "child-notify::title", G_CALLBACK(on_child_updated),
                   sidebar);
  ...
  g_signal_connect(widget, "child-notify::needs-attention",
                   G_CALLBACK(on_child_updated), sidebar);
  g_signal_connect(widget, "notify::visible", G_CALLBACK(on_child_updated),
                   sidebar);
...
static void remove_child(GtkWidget *widget, XAppStackSidebar *sidebar) {
  GtkWidget *row;
...
static void populate_sidebar(XAppStackSidebar *sidebar) {
  GtkWidget *widget;
  GtkWidget *row;

  gtk_container_foreach(GTK_CONTAINER(sidebar->stack), (GtkCallback)add_child,
                        sidebar);
...
static void clear_sidebar(XAppStackSidebar *sidebar) {
  gtk_container_foreach(GTK_CONTAINER(sidebar->stack),
                        (GtkCallback)remove_child, sidebar);
}

static void on_child_changed(GtkWidget *widget, GParamSpec *pspec,
                             XAppStackSidebar *sidebar) {
  GtkWidget *child;
  GtkWidget *row;
...
static void on_stack_child_added(GtkContainer *container, GtkWidget *widget,
                                 XAppStackSidebar *sidebar) {
  add_child(widget, sidebar);
}

static void on_stack_child_removed(GtkContainer *container, GtkWidget *widget,
                                   XAppStackSidebar *sidebar) {
  remove_child(widget, sidebar);
}

static void disconnect_stack_signals(XAppStackSidebar *sidebar) {
  g_signal_handlers_disconnect_by_func(sidebar->stack, on_stack_child_added,
                                       sidebar);
  g_signal_handlers_disconnect_by_func(sidebar->stack, on_stack_child_removed,
                                       sidebar);
  g_signal_handlers_disconnect_by_func(sidebar->stack, on_child_changed,
                                       sidebar);
  g_signal_handlers_disconnect_by_func(sidebar->stack, disconnect_stack_signals,
                                       sidebar);
}

static void connect_stack_signals(XAppStackSidebar *sidebar) {
...
static void xapp_stack_sidebar_dispose(GObject *object) {
  XAppStackSidebar *sidebar = XAPP_STACK_SIDEBAR(object);
...
static void xapp_stack_sidebar_finalize(GObject *object) {
  XAppStackSidebar *sidebar = XAPP_STACK_SIDEBAR(object);
...
static void xapp_stack_sidebar_class_init(XAppStackSidebarClass *klass) {
  ...
  obj_properties[PROP_STACK] = g_param_spec_object(
      "stack", "Stack", "Associated stack for this XAppStackSidebar",
      GTK_TYPE_STACK,
...
void xapp_stack_sidebar_set_stack(XAppStackSidebar *sidebar, GtkStack *stack) {
