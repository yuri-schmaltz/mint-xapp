...
enum { PROP_0, PROP_ICON_SIZE, PROP_ICON, PROP_CATEGORY, N_PROPERTIES };

static GParamSpec *obj_properties[N_PROPERTIES] = {
    NULL,
};

G_DEFINE_TYPE_WITH_PRIVATE(XAppIconChooserButton, xapp_icon_chooser_button,
                           GTK_TYPE_BUTTON)

static void ensure_dialog(XAppIconChooserButton *button) {
...
static void on_clicked(GtkButton *button) {
  ...
  priv = xapp_icon_chooser_button_get_instance_private(
      XAPP_ICON_CHOOSER_BUTTON(button));
  ...
  if (priv->category_string) {
    response = xapp_icon_chooser_dialog_run_with_category(
        priv->dialog, priv->category_string);
  } else if (priv->icon_string) {
    response =
        xapp_icon_chooser_dialog_run_with_icon(priv->dialog, priv->icon_string);
  } else {
...
void xapp_icon_chooser_button_get_property(GObject *object, guint prop_id,
...
void xapp_icon_chooser_button_set_property(GObject *object, guint prop_id,
                                           const GValue *value,
                                           GParamSpec *pspec) {
  XAppIconChooserButton *button;
  ...
  case PROP_ICON:
    ...
    break;
  case PROP_CATEGORY:
    xapp_icon_chooser_button_set_default_category(button,
...
void xapp_icon_chooser_button_dispose(GObject *object) {
  XAppIconChooserButtonPrivate *priv;

  priv = xapp_icon_chooser_button_get_instance_private(
      XAPP_ICON_CHOOSER_BUTTON(object));
...
static void xapp_icon_chooser_button_init(XAppIconChooserButton *button) {
  XAppIconChooserButtonPrivate *priv;

  priv = xapp_icon_chooser_button_get_instance_private(button);

  priv->image =
      gtk_image_new_from_icon_name("unknown", XAPP_BUTTON_ICON_SIZE_DEFAULT);
...
xapp_icon_chooser_button_class_init(XAppIconChooserButtonClass *klass) {
  ...
  obj_properties[PROP_ICON_SIZE] = g_param_spec_enum(
      ...
      GTK_TYPE_ICON_SIZE, GTK_ICON_SIZE_DND,
  ...
  obj_properties[PROP_ICON] = g_param_spec_string(
      "icon", _("Icon"), _("The string representing the icon."), "",
      G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY);
  ...
  obj_properties[PROP_CATEGORY] =
      g_param_spec_string("category", _("Category"), _("The default category."),
                          "", G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY);
 ...
 * @icon_size: the size of icon to use in the button, or NULL to use the default
 * value.
 *
 * Creates a new #XAppIconChooserButton, and sets the sizes of the button image
 * and the icons in the dialog. Note that xapp_icon_chooser_button_new_with_size
 * (NULL, NULL) is the same as calling xapp_icon_chooser_button_new ().
 *
 * Returns: a newly created #XAppIconChooserButton
 */
 ...
 * @icon_size: the size of icon to use in the button, or -1 to use the default
 * value.
 *
...
void xapp_icon_chooser_button_set_icon_size(XAppIconChooserButton *button,
  ...
  xapp_icon_chooser_button_set_icon(button, priv->icon_string);
 ...
 *
 * returns: a string representing the icon. This may be an icon name or a file
 * path.
 */
 ...
 * xapp_icon_chooser_button_set_icon:
 * @button: a #XAppIconChooserButton
 * @icon: (nullable): a string representing the icon to be set. This may be an
 * icon name or a file path.
 *
...
void xapp_icon_chooser_button_set_icon(XAppIconChooserButton *button,
  ...
  if (g_strcmp0(icon, priv->icon_string) == 0) {
    return;
  }
  ...
  if (g_strrstr(icon_string, "/")) {
    ...
    gtk_image_set_from_icon_name(GTK_IMAGE(priv->image), icon_string,
                                 priv->icon_size);
  }
 ...
 * @button: a #XAppIconChooserButton
 * @category: (nullable): a string representing the category selected by
 * default.
 *
 * Sets the icon on the #XAppIconChooserButton.
 */
void xapp_icon_chooser_button_set_default_category(
    XAppIconChooserButton *button, const gchar *category) {
  XAppIconChooserButtonPrivate *priv;
