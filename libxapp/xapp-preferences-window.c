...
G_DEFINE_TYPE_WITH_PRIVATE(XAppPreferencesWindow, xapp_preferences_window,
                           GTK_TYPE_WINDOW)

static void xapp_preferences_window_init(XAppPreferencesWindow *window) {
  XAppPreferencesWindowPrivate *priv =
      xapp_preferences_window_get_instance_private(window);
  GtkWidget *main_box;
  ...
  gtk_box_pack_start(GTK_BOX(secondary_box), GTK_WIDGET(priv->side_switcher),
                     FALSE, FALSE, 0);
  ...
  atk_object_set_name(gtk_widget_get_accessible(priv->stack), _("Pages"));
  atk_object_set_name(
      gtk_widget_get_accessible(GTK_WIDGET(priv->side_switcher)),
      _("Preference Categories"));

  /* Keep track of the number of pages so we can hide the stack switcher with <
   * 2 */
  priv->num_pages = 0;
}

static void xapp_preferences_window_close(XAppPreferencesWindow *window) {
...
xapp_preferences_window_class_init(XAppPreferencesWindowClass *klass) {
  ...
  signals[CLOSE] = g_signal_new(
      ...
      G_STRUCT_OFFSET(XAppPreferencesWindowClass, close), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
...
void xapp_preferences_window_add_button(XAppPreferencesWindow *window,
  ...
  if (pack_type == GTK_PACK_END) {
    gtk_button_box_set_child_secondary(GTK_BUTTON_BOX(priv->button_area),
                                       button, TRUE);
