...
G_DEFINE_TYPE_WITH_PRIVATE(XAppIconChooserDialog, xapp_icon_chooser_dialog,
...
static void on_select_button_clicked(GtkButton *button, gpointer user_data);

static void on_cancel_button_clicked(GtkButton *button, gpointer user_data);

static void on_default_button_clicked(GtkButton *button, gpointer user_data);

static gboolean on_search_bar_key_pressed(GtkWidget *widget, GdkEvent *event,
                                          gpointer user_data);

static gboolean on_delete_event(GtkWidget *widget, GdkEventAny *event);
...
static void search_path(XAppIconChooserDialog *dialog, const gchar *path_string,
...
static gint list_box_sort(GtkListBoxRow *row1, GtkListBoxRow *row2,
                          gpointer user_data);

static gint search_model_sort(GtkTreeModel *model, GtkTreeIter *a,
                              GtkTreeIter *b, gpointer user_data);

static void free_category_info(IconCategoryInfo *category_info) {
...
static void free_file_info(FileIconInfoLoadCallbackInfo *file_info) {
...
static void free_named_info(NamedIconInfoLoadCallbackInfo *named_info) {
...
#if DEBUG_REFS
static void on_cancellable_finalize(gpointer data, GObject *object) {
  g_printerr("Cancellable Finalize: %p\n", object);
}

static void on_enumerator_finalize(gpointer data, GObject *object) {
  g_printerr("Enumerator Finalize: %p\n", object);
}
#endif

static void on_enumerator_toggle_ref_called(gpointer data, GObject *object,
  ...
  if (is_last_ref) {
    g_object_remove_toggle_ref(
        object, (GToggleNotify)on_enumerator_toggle_ref_called, dialog);

    priv->search_file_enumerator = NULL;
  }
}

static void clear_search_state(XAppIconChooserDialog *dialog) {
  XAppIconChooserDialogPrivate *priv;
...
void xapp_icon_chooser_dialog_get_property(GObject *object, guint prop_id,
  ...
  case PROP_DEFAULT_ICON:
    g_value_set_string(value,
                       xapp_icon_chooser_dialog_get_default_icon(dialog));
    break;
  default:
...
void xapp_icon_chooser_dialog_set_property(GObject *object, guint prop_id,
                                           const GValue *value,
                                           GParamSpec *pspec) {
  XAppIconChooserDialog *dialog;
  XAppIconChooserDialogPrivate *priv;
  ...
  case PROP_ALLOW_PATHS:
    ...
    break;
  case PROP_DEFAULT_ICON:
    xapp_icon_chooser_dialog_set_default_icon(dialog,
...
static void xapp_icon_chooser_dialog_dispose(GObject *object) {
  XAppIconChooserDialog *dialog;
  XAppIconChooserDialogPrivate *priv;
...
static void xapp_icon_chooser_dialog_init(XAppIconChooserDialog *dialog) {
  XAppIconChooserDialogPrivate *priv;
  GtkWidget *main_box;
  ...
  priv->categories = g_hash_table_new_full(NULL, NULL, NULL,
                                           (GDestroyNotify)free_category_info);

  /* surfaces_by_name will save surfaces generated by icon name, so they can
   * avoid creating them again when they're reloaded (like re-selecting a
   * previously selected category) */
  priv->surfaces_by_name = g_hash_table_new_full(
  ...
  priv->search_icon_store = gtk_list_store_new(3, G_TYPE_STRING, G_TYPE_STRING,
                                               CAIRO_GOBJECT_TYPE_SURFACE);

  gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(priv->search_icon_store),
                                  COLUMN_DISPLAY_NAME, search_model_sort, priv,
                                  NULL);
  gtk_tree_sortable_set_sort_column_id(
      GTK_TREE_SORTABLE(priv->search_icon_store), COLUMN_DISPLAY_NAME,
      GTK_SORT_ASCENDING);
  ...
  gtk_box_pack_start(GTK_BOX(toolbar_box), priv->browse_button, FALSE, FALSE,
                     0);
  ...
  gtk_list_box_set_sort_func(GTK_LIST_BOX(priv->list_box), list_box_sort, NULL,
                             NULL);
  ...
  g_object_set(loading_bar_box, "margin", 4, NULL);
  ...
  gtk_orientable_set_orientation(GTK_ORIENTABLE(ca_box),
  ...
  gtk_cell_area_attribute_connect(GTK_CELL_AREA(ca_box), r, "surface",
                                  COLUMN_SURFACE);

  r = gtk_cell_renderer_text_new();
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(ca_box), r, FALSE);
  gtk_cell_area_attribute_connect(GTK_CELL_AREA(ca_box), r, "text",
                                  COLUMN_DISPLAY_NAME);
  g_object_set(G_OBJECT(r), "alignment", PANGO_ALIGN_CENTER, "xalign", 0.5,
  ...
  gtk_container_add(GTK_CONTAINER(scrolled_window),
                    GTK_WIDGET(priv->icon_view));

  gtk_icon_view_set_tooltip_column(GTK_ICON_VIEW(priv->icon_view),
                                   COLUMN_FULL_NAME);
  ...
  atk_object_set_name(gtk_widget_get_accessible(priv->search_bar),
                      _("Search icons"));
  atk_object_set_name(gtk_widget_get_accessible(GTK_WIDGET(priv->list_box)),
                      _("Icon categories"));
  atk_object_set_name(gtk_widget_get_accessible(GTK_WIDGET(priv->icon_view)),
                      _("Icons"));
...
xapp_icon_chooser_dialog_class_init(XAppIconChooserDialogClass *klass) {
   ...
   *
   * The preferred size to use when looking up icons. This only works with icon
   * names. Additionally, there is no guarantee that a selected icon name will
   * exist in a particular size.
   */
  obj_properties[PROP_ICON_SIZE] = g_param_spec_enum(
      "icon-size", _("Icon size"), _("The preferred icon size."),
      XAPP_TYPE_ICON_SIZE, XAPP_ICON_SIZE_32,
   ...
   */
  obj_properties[PROP_ALLOW_PATHS] = g_param_spec_boolean(
      "allow-paths", _("Allow Paths"), _("Whether to allow paths."), TRUE,
      G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY);
   ...
   */
  obj_properties[PROP_DEFAULT_ICON] = g_param_spec_string(
      "default-icon", _("Default Icon"), _("The icon to use by default"), NULL,
      G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY);

  g_object_class_install_properties(object_class, N_PROPERTIES, obj_properties);

  // keybinding signals
  signals[CLOSE] = g_signal_new("close", G_TYPE_FROM_CLASS(klass),
                                G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                                G_STRUCT_OFFSET(GtkWidgetClass, delete_event),
                                NULL, NULL, NULL, G_TYPE_NONE, 0);

  signals[SELECT] = g_signal_new("select", G_TYPE_FROM_CLASS(klass),
                                 G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, 0, NULL,
 ...
 *
 * Shows the dialog and enters a separate main loop until an icon is chosen or
 * the action is canceled.
 *
 * xapp_icon_chooser_dialog_run (), xapp_icon_chooser_dialog_run_with_icon(),
 * and xapp_icon_chooser_dialog_run_with_category () may all be called multiple
 * times. This is useful for applications which use this dialog multiple times,
 * as it may improve performance for subsequent calls.
 *
 * Returns: GTK_RESPONSE_OK if the user selected an icon, or GTK_RESPONSE_CANCEL
 * otherwise
 */
 ...
 * This can be either an icon name or a path. Passing an icon string or path
 * that doesn't exist is accepted, but it may show multiple results, or none at
 * all. This behavior is useful if, for example, you wish to have the user
 * select an image file from a particular directory.
 *
 * If the property allow_paths is FALSE, setting a path will yield no results
 ...
 * @dialog: a #XAppIconChooserDialog
 *
 * Like xapp_icon_chooser_dialog_run but selects a particular category specified
 * by @category. This is used when there is a particular category of icon that
 * is more appropriate than the others. If the category does not exist, the
 * first category in the list will be selected. To get a list of possible
 * categories, use gtk_icon_theme_list_contexts ().
 *
 * xapp_icon_chooser_dialog_run (), xapp_icon_chooser_dialog_run_with_icon(),
 * and xapp_icon_chooser_dialog_run_with_category () may all be called multiple
 * times. This is useful for applications which use this dialog multiple times,
...
gint xapp_icon_chooser_dialog_run_with_category(XAppIconChooserDialog *dialog,
  ...
  for (; children; children = children->next) {
    ...
    if (g_strcmp0(context, category) == 0) {
      gtk_list_box_select_row(GTK_LIST_BOX(priv->list_box),
 ...
 * @icons: (transfer full) (element-type utf8): a list of icon names to add to
 * the new category
 *
...
void xapp_icon_chooser_dialog_add_custom_category(XAppIconChooserDialog *dialog,
  ...
  category_info->model = gtk_list_store_new(3, G_TYPE_STRING, G_TYPE_STRING,
                                            CAIRO_GOBJECT_TYPE_SURFACE);
  g_signal_connect(category_info->model, "row-inserted",
                   G_CALLBACK(on_icon_store_icons_added), dialog);

  category_info->icons =
      g_list_sort(category_info->icons, (GCompareFunc)g_utf8_collate);
 ...
 * @dialog: a #XAppIconChooserDialog
 *
 * Gets the currently selected icon from the dialog. If allow-paths is TRUE,
 * this function may return either an icon name or a path depending on what the
 * user selects. Otherwise it will only return an icon name.
 *
 * Returns: (transfer full): the string representation of the currently selected
 * icon or NULL if no icon is selected.
 */
...
static void xapp_icon_chooser_dialog_close(XAppIconChooserDialog *dialog,
                                           GtkResponseType response) {
  XAppIconChooserDialogPrivate *priv;
...
static void on_custom_button_clicked(GtkButton *button, gpointer user_data) {
  ...
  response_id =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(button), "response-id"));

  xapp_icon_chooser_dialog_close(XAPP_ICON_CHOOSER_DIALOG(user_data),
                                 response_id);
}
...
void xapp_icon_chooser_dialog_add_button(XAppIconChooserDialog *dialog,
                                         GtkWidget *button, GtkPackType packing,
                                         GtkResponseType response_id) {
  XAppIconChooserDialogPrivate *priv;

  priv = xapp_icon_chooser_dialog_get_instance_private(dialog);

  g_signal_connect(button, "clicked", G_CALLBACK(on_custom_button_clicked),
                   dialog);

  /* This saves having to use a custom container for callback data. */
  g_object_set_data(G_OBJECT(button), "response-id",
...
static gint search_model_sort(GtkTreeModel *model, GtkTreeIter *a,
  ...
  XAppIconChooserDialogPrivate *priv =
      (XAppIconChooserDialogPrivate *)user_data;
  ...
  if (search_str == NULL) {
  } else if (g_strcmp0(a_value, search_str) == 0) {
    ret = -1;
  } else if (g_strcmp0(b_value, search_str) == 0) {
    ret = 1;
  } else {
...
static void load_categories(XAppIconChooserDialog *dialog) {
  ...
  for (i = 0; i < G_N_ELEMENTS(categories); i++) {
    ...
    for (j = 0; category->contexts[j] != NULL; j++) {
      ...
      match = g_list_find_custom(contexts, category->contexts[j],
    ...
    /* Any contexts not consumed by categories should be added to the 'other'
     * category */
    if (i == (G_N_ELEMENTS(categories) - 1) && g_list_length(contexts) > 0) {
      for (l = contexts; l != NULL; l = l->next) {

#if DEBUG_ICON_THEME
        g_message("Adding unused category to Other category: '%s'",
                  (gchar *)l->data);
#endif
        context_icons = gtk_icon_theme_list_icons(theme, (gchar *)l->data);

        category_info->icons =
            g_list_concat(category_info->icons, context_icons);
      }
    }
    ...
    priv->full_icon_list =
        g_list_concat(priv->full_icon_list, g_list_copy(category_info->icons));
  ...
  priv->full_icon_list =
      g_list_sort(priv->full_icon_list, (GCompareFunc)g_utf8_collate);

  row = gtk_list_box_get_row_at_index(GTK_LIST_BOX(priv->list_box), 0);
  gtk_list_box_select_row(GTK_LIST_BOX(priv->list_box), row);
}

static gboolean load_next_file_search_chunk(gpointer user_data) {
  XAppIconChooserDialogPrivate *priv;
  ...
  search_path(callback_info->dialog, priv->current_text,
...
static gboolean load_next_category_chunk(gpointer user_data) {
  XAppIconChooserDialogPrivate *priv;
  ...
  load_icons_for_category(callback_info->dialog, callback_info->category_info,
                          priv->icon_size);
...
static gboolean load_next_name_search_chunk(gpointer user_data) {
  XAppIconChooserDialogPrivate *priv;
  NamedIconInfoLoadCallbackInfo *callback_info;
  ...
  search_icon_name(callback_info->dialog, priv->current_text,
                   priv->search_icon_store);
...
static void finish_pixbuf_load_from_file(GObject *stream, GAsyncResult *res,
                                         gpointer *user_data) {
  FileIconInfoLoadCallbackInfo *callback_info;
  GdkPixbuf *pixbuf;
  GError *error = NULL;
  ...
  if (pixbuf) {
    ...
    surface = gdk_cairo_surface_create_from_pixbuf(
        pixbuf, 0, gtk_widget_get_window(GTK_WIDGET(callback_info->dialog)));

    gtk_list_store_append(callback_info->model, &iter);
    gtk_list_store_set(callback_info->model, &iter, COLUMN_DISPLAY_NAME,
                       callback_info->short_name, COLUMN_FULL_NAME,
                       callback_info->long_name, COLUMN_SURFACE, surface, -1);
...
static void add_named_entry(NamedIconInfoLoadCallbackInfo *callback_info,
                            cairo_surface_t *surface) {
  GtkTreeIter iter;

  gtk_list_store_append(callback_info->model, &iter);
  gtk_list_store_set(callback_info->model, &iter, COLUMN_DISPLAY_NAME,
                     callback_info->name, COLUMN_FULL_NAME, callback_info->name,
                     COLUMN_SURFACE, surface, -1);
}

static gboolean add_named_entry_with_existing_surface(gpointer user_data) {
  NamedIconInfoLoadCallbackInfo *callback_info;
...
static void finish_pixbuf_load_from_name(GObject *info, GAsyncResult *res,
                                         gpointer *user_data) {
  XAppIconChooserDialogPrivate *priv;
  NamedIconInfoLoadCallbackInfo *callback_info;
  GdkPixbuf *pixbuf;
  ...
  pixbuf = gtk_icon_info_load_symbolic_for_context_finish(GTK_ICON_INFO(info),
                                                          res, NULL, &error);
  g_object_unref(info);
  ...
  g_hash_table_insert(priv->surfaces_by_name, g_strdup(callback_info->name),
                      (gpointer)surface);
  ...
  for (chunk_count = 0; chunk_count < CATEGORY_CHUNK_SIZE; chunk_count++) {
    ...
    if (category_info->iter) {
      ...
      if (surface != NULL) {
        ...
        g_idle_add((GSourceFunc)add_named_entry_with_existing_surface,
                   callback_info);
      } else {
        GtkIconInfo *info;
        ...
        info = gtk_icon_theme_lookup_icon_for_scale(
            theme, name, icon_size, scale, GTK_ICON_LOOKUP_FORCE_SIZE);
        if (info == NULL) {
          // this shouldn't be the case most of the time, but if a custom
          // category is defined it's possible the icon doesn't exist. In that
          // case it's best to just skip over it since trying to load it will
          // lead to a segfault.
          continue;
        }
        ...
        gtk_icon_info_load_symbolic_for_context_async(
            info, context, NULL,
            (GAsyncReadyCallback)(finish_pixbuf_load_from_name), callback_info);
      }
    } else {
      gtk_widget_hide(priv->loading_bar);
...
static void on_category_selected(GtkListBox *list_box,
...
#if DEBUG_REFS
  g_object_weak_ref(G_OBJECT(priv->cancellable),
                    (GWeakNotify)on_cancellable_finalize, priv);
#endif
  ...
  gtk_icon_view_set_model(GTK_ICON_VIEW(priv->icon_view),
                          GTK_TREE_MODEL(category_info->model));

  load_icons_for_category(dialog, category_info, priv->icon_size);

  gtk_adjustment_set_value(
      gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(priv->icon_view)), 0.0);
...
static void search_path(XAppIconChooserDialog *dialog, const gchar *path_string,
  ...
  if (!priv->search_file_enumerator) {
    priv->search_file_enumerator = g_file_enumerate_children(
        dir,
...
#if DEBUG_REFS
    g_object_weak_ref(G_OBJECT(priv->search_file_enumerator),
                      (GWeakNotify)on_enumerator_finalize, dialog);
#endif
  }
  ...
  while (chunk_count < SEARCH_CHUNK_SIZE) {
    ...
    child_info =
        g_file_enumerator_next_file(priv->search_file_enumerator, NULL, NULL);
    ...
    child =
        g_file_enumerator_get_child(priv->search_file_enumerator, child_info);
    ...
    if (search_str == NULL || g_str_has_prefix(child_name, search_str)) {
      ...
      content_type = g_file_info_get_attribute_string(
          child_info, G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE);
      if (content_type == NULL) {
        content_type = g_file_info_get_attribute_string(
            child_info, G_FILE_ATTRIBUTE_STANDARD_FAST_CONTENT_TYPE);
      }
      ...
      if (content_type && g_content_type_is_a(content_type, "image/*")) {
        ...
        if (stream != NULL) {
          ...
          callback_info->cancellable = g_object_ref(priv->cancellable);
          callback_info->enumerator =
              g_object_ref(priv->search_file_enumerator);
          ...
          gdk_pixbuf_new_from_stream_at_scale_async(
              G_INPUT_STREAM(stream), -1, priv->icon_size * scale, TRUE, NULL,
              (GAsyncReadyCallback)finish_pixbuf_load_from_file, callback_info);
    ...
    if (priv->search_iter) {
      ...
      if (g_strrstr(casefolded, name_string)) {
        ...
        if (surface != NULL) {
          ...
          info = gtk_icon_theme_lookup_icon_for_scale(
              theme, name, priv->icon_size, scale, GTK_ICON_LOOKUP_FORCE_SIZE);
          context = gtk_widget_get_style_context(priv->icon_view);
          gtk_icon_info_load_symbolic_for_context_async(
              info, context, NULL,
              (GAsyncReadyCallback)(finish_pixbuf_load_from_name),
              callback_info);
        }
...
static void on_search_text_changed(GtkSearchEntry *entry,
...
#if DEBUG_REFS
  g_object_weak_ref(G_OBJECT(priv->cancellable),
                    (GWeakNotify)on_cancellable_finalize, dialog);
#endif
  ...
  if (g_strcmp0(search_text, "") == 0) {
    ...
    gtk_list_store_clear(GTK_LIST_STORE(priv->search_icon_store));
    gtk_icon_view_set_model(GTK_ICON_VIEW(priv->icon_view),
                            GTK_TREE_MODEL(priv->search_icon_store));
...
static gboolean on_delete_event(GtkWidget *widget, GdkEventAny *event) {
  xapp_icon_chooser_dialog_close(XAPP_ICON_CHOOSER_DIALOG(widget),
                                 GTK_RESPONSE_CANCEL);
