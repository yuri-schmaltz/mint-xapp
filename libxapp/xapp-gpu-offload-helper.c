...
static const gchar *DEFAULT_ENV_ARGV[5] = {"__NV_PRIME_RENDER_OFFLOAD", "1",
...
G_DEFINE_BOXED_TYPE(XAppGpuInfo, xapp_gpu_info, xapp_gpu_info_copy,
                    xapp_gpu_info_free);
/**
 ...
 *
 * #XAppGpuOffloadHelper is class that provides a reliable property cache and
 * simple return methods for getting offload_helper parameters and conditions
 * from the Switcheroo interface.
 *
 * Since 2.6
 */
...
enum { READY, LAST_SIGNAL };

static guint signals[LAST_SIGNAL] = {
    0,
};

static void process_gpus_property(XAppGpuOffloadHelper *helper,
                                  GError **error) {
  GVariant *gpus;
  GList *infos = NULL;
  ...
  gpus = xapp_switcheroo_control_get_gpus(
      XAPP_SWITCHEROO_CONTROL(helper->control));
  ...
  if (infos == NULL) {
    *error = g_error_new(g_dbus_error_quark(), G_DBUS_ERROR_FAILED,
...
static void populate_gpu_info(XAppGpuOffloadHelper *helper, GError **error) {
  GVariant *gpus;
  g_auto(GStrv) cached_props = NULL;

  cached_props = g_dbus_proxy_get_cached_property_names(helper->control);

  if (cached_props != NULL &&
      g_strv_contains((const gchar *const *)cached_props, "GPUs")) {
    // (already cached)
  ...
  gpus = g_dbus_connection_call_sync(
      g_dbus_proxy_get_connection(helper->control),
      g_dbus_proxy_get_name(helper->control),
      g_dbus_proxy_get_object_path(helper->control),
      "org.freedesktop.DBus.Properties", "Get",
      g_variant_new("(ss)", g_dbus_proxy_get_interface_name(helper->control),
                    "GPUs"),
      NULL, G_DBUS_CALL_FLAGS_NONE, 5000, helper->cancellable, error);
...
static void on_bus_connection_closed(GDBusConnection *connection,
                                     gboolean remote_peer_vanished,
                                     GError *error, gpointer user_data) {
  XAppGpuOffloadHelper *helper = XAPP_GPU_OFFLOAD_HELPER(user_data);

  if (error != NULL) {
    g_critical("Bus connection unexpectedly lost: (%d) %s", error->code,
               error->message);
    g_error_free(error);
  }
...
static void helper_init_thread(GTask *task, gpointer source_object,
                               gpointer task_data, GCancellable *cancellable) {
  XAppGpuOffloadHelper *helper = XAPP_GPU_OFFLOAD_HELPER(source_object);
  XAppSwitcherooControl *control;
  GError *error = NULL;

  control = xapp_switcheroo_control_proxy_new_for_bus_sync(
  ...
  if (error != NULL) {
    ...
    g_critical("Could not create switcheroo proxy: (%d): %s", error->code,
               error->message);
  ...
  if (g_dbus_proxy_get_name_owner(G_DBUS_PROXY(control)) != NULL) {
    ...
    g_signal_connect(g_dbus_proxy_get_connection(helper->control), "closed",
                     G_CALLBACK(on_bus_connection_closed), helper);
    ...
    if (error != NULL) {
      ...
      g_critical("Could not get gpu info from switcheroo proxy: (%d): %s",
                 error->code, error->message);
      g_task_return_error(task, error);
    }
  } else {
    g_warning("Switcheroo-control doesn't appear to be running, checking for "
              "Ubuntu support...");
...
static void helper_task_callback(GObject *source_object, GAsyncResult *res,
                                 gpointer user_data) {
  XAppGpuOffloadHelper *helper = XAPP_GPU_OFFLOAD_HELPER(source_object);
...
static void init_helper(XAppGpuOffloadHelper *helper, gboolean synchronous) {
  GTask *task =
      g_task_new(helper, helper->cancellable, helper_task_callback, NULL);
...
static void xapp_gpu_offload_helper_init(XAppGpuOffloadHelper *helper) {
  helper->cancellable = g_cancellable_new();
  g_mutex_init(&helper->gpu_infos_mutex);
}

static void xapp_gpu_offload_helper_dispose(GObject *object) {
...
xapp_gpu_offload_helper_class_init(XAppGpuOffloadHelperClass *klass) {
   ...
   */
  signals[READY] = g_signal_new("ready", XAPP_TYPE_GPU_OFFLOAD_HELPER,
                                G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, 0, NULL,
                                NULL, NULL, G_TYPE_NONE, 1, G_TYPE_BOOLEAN);
}

static gboolean idle_init_helper(gpointer user_data) {
  XAppGpuOffloadHelper *helper = XAPP_GPU_OFFLOAD_HELPER(user_data);
...
static XAppGpuOffloadHelper *helper_get_common(gboolean synchronous) {
 ...
 *
 * The #XAppGpuOffloadHelper::ready signal will be emitted when the helper is
 ...
 *
 * Creates a new #XAppGpuOffloadHelper instance. This performs initialization
 * synchronously, and can potentially block.
 *
 * Use xapp_gpu_offload_helper_is_ready() to see if the helper was initialized
 * successfully.
 *
...
static void warn_if_not_ready(XAppGpuOffloadHelper *helper) {
  if (!helper->ready) {
 ...
 *
 * Returns: (element-type XAppGpuInfo) (transfer container): a list of
 * #XAppGpuInfos or %NULL if there is only a single GPU. The elements are owned
 * by @helper but the container itself should be freed.
 *
 * Since: 2.6
 */
 ...
 *
 * Returns: (transfer none): the appropriate #XAppGpuInfo, or %NULL if @id was
 * invalid.
 *
 * Since: 2.6
 */
