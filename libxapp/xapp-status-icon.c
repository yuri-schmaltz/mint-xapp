...
static void cancellable_reset(XAppStatusIcon *self) {
...
static const gchar *panel_position_to_str(GtkPositionType type) {
  switch (type) {
  case GTK_POS_LEFT:
    return "Left";
  case GTK_POS_RIGHT:
...
static const gchar *button_to_str(guint button) {
  switch (button) {
...
static const gchar *state_to_str(XAppStatusIconState state) {
  switch (state) {
...
static const gchar *direction_to_str(XAppScrollDirection direction) {
  switch (direction) {
  case XAPP_SCROLL_UP:
    return "Up";
...
static GdkEvent *synthesize_event(XAppStatusIcon *self, gint x, gint y,
                                  guint button, guint _time, gint position,
...
static void primary_menu_unmapped(GtkWidget *widget, gpointer user_data) {
  ...
  if (process_icon_state == XAPP_STATUS_ICON_STATE_NATIVE) {
    xapp_status_icon_interface_set_primary_menu_is_open(
        icon->priv->interface_skeleton, FALSE);
  }

  g_signal_handlers_disconnect_by_func(widget, primary_menu_unmapped, icon);
}

static void secondary_menu_unmapped(GtkWidget *widget, gpointer user_data) {
  g_return_if_fail(XAPP_IS_STATUS_ICON(user_data));
  XAppStatusIcon *icon = XAPP_STATUS_ICON(user_data);
...
static void popup_menu(XAppStatusIcon *self, GtkMenu *menu, gint x, gint y,
                       guint button, guint _time, gint panel_position) {
  GdkWindow *rect_window;
  GdkEvent *event;
  GdkRectangle win_rect;
  ...
  if (button == GDK_BUTTON_PRIMARY) {
    if (process_icon_state == XAPP_STATUS_ICON_STATE_NATIVE) {
      xapp_status_icon_interface_set_primary_menu_is_open(
          self->priv->interface_skeleton, TRUE);
    }

    g_signal_connect(gtk_widget_get_toplevel(GTK_WIDGET(menu)), "unmap",
                     G_CALLBACK(primary_menu_unmapped), self);
    ...
    g_signal_connect(gtk_widget_get_toplevel(GTK_WIDGET(menu)), "unmap",
                     G_CALLBACK(secondary_menu_unmapped), self);
  }

  event = synthesize_event(self, x, y, button, _time, panel_position,
                           &rect_window, &win_rect, &rect_anchor, &menu_anchor);

  g_object_set_data_full(G_OBJECT(menu), "rect_window", rect_window,
                         (GDestroyNotify)gdk_window_destroy);

  g_object_set(G_OBJECT(menu), "anchor-hints",
               GDK_ANCHOR_SLIDE_X | GDK_ANCHOR_SLIDE_Y | GDK_ANCHOR_RESIZE_X |
                   GDK_ANCHOR_RESIZE_Y,
               NULL);

  gtk_menu_popup_at_rect(menu, rect_window, &win_rect, rect_anchor, menu_anchor,
                         event);

  gdk_event_free(event);
}

static gboolean should_send_activate(XAppStatusIcon *icon, guint button) {
...
static GtkWidget *get_menu_to_use(XAppStatusIcon *icon, guint button) {
...
static gboolean handle_click_method(XAppStatusIconInterface *skeleton,
                                    GDBusMethodInvocation *invocation, gint x,
                                    gint y, guint button, guint _time,
                                    gint panel_position, XAppStatusIcon *icon) {
  ...
  if (g_strcmp0(name, "ButtonPress") == 0) {
    DEBUG("Received ButtonPress from monitor %s: "
          ...
          g_dbus_method_invocation_get_sender(invocation), x, y,
          button_to_str(button), _time, panel_position_to_str(panel_position));

    if (should_send_activate(icon, button)) {
      DEBUG("Native sending 'activate' for %s button", button_to_str(button));
      g_signal_emit(icon, signals[ACTIVATE], 0, button, _time);
    }

    icon->priv->have_button_press = TRUE;

    g_signal_emit(icon, signals[BUTTON_PRESS], 0, x, y, button, _time,
                  panel_position);

    xapp_status_icon_interface_complete_button_press(skeleton, invocation);
  } else if (g_strcmp0(name, "ButtonRelease") == 0) {
    ...
    if (icon->priv->have_button_press) {
      ...
      if (menu_to_use) {
        popup_menu(icon, GTK_MENU(menu_to_use), x, y, button, _time,
                   panel_position);
      }

      g_signal_emit(icon, signals[BUTTON_RELEASE], 0, x, y, button, _time,
                    panel_position);
    }

    icon->priv->have_button_press = FALSE;

    xapp_status_icon_interface_complete_button_release(skeleton, invocation);
  }
...
static gboolean handle_scroll_method(XAppStatusIconInterface *skeleton,
                                     ...
                                     gint delta, XAppScrollDirection direction,
                                     guint _time, XAppStatusIcon *icon) {
  DEBUG("Received Scroll from monitor %s: "
        "delta: %d , direction: %s , time: %u",
        g_dbus_method_invocation_get_sender(invocation), delta,
        direction_to_str(direction), _time);

  g_signal_emit(icon, signals[SCROLL], 0, delta, direction, _time);

  xapp_status_icon_interface_complete_scroll(skeleton, invocation);

  return TRUE;
}

static void calculate_gtk_status_icon_position_and_orientation(
    XAppStatusIcon *icon, GtkStatusIcon *status_icon, gint *x, gint *y,
    gint *orientation) {
  GdkScreen *screen;
  GdkRectangle irect;
  ...
  if (gtk_status_icon_get_geometry(status_icon, &screen, &irect,
                                   &iorientation)) {
    ...
    monitor = gdk_display_get_monitor_at_point(
        display, irect.x + (irect.width / 2), irect.y + (irect.height / 2));
...
static gboolean on_gtk_status_icon_button_press(GtkStatusIcon *status_icon,
                                                GdkEvent *event,
                                                gpointer user_data) {
  ...
  DEBUG("GtkStatusIcon button-press-event with %s button",
  ...
  calculate_gtk_status_icon_position_and_orientation(icon, status_icon, &x, &y,
                                                     &orientation);
...
static gboolean on_gtk_status_icon_button_release(GtkStatusIcon *status_icon,
                                                  GdkEvent *event,
                                                  gpointer user_data) {
  XAppStatusIcon *icon = user_data;
  ...
  DEBUG("GtkStatusIcon button-release-event with %s button",
        button_to_str(button));
  ...
  if (menu_to_use) {
    ...
    popup_menu(icon, GTK_MENU(menu_to_use), x, y, button, _time, orientation);
  }
...
static gboolean on_gtk_status_icon_scroll(GtkStatusIcon *status_icon,
  ...
  if (gdk_event_get_scroll_direction(event, &direction)) {
    ...
    DEBUG("Received Scroll from GtkStatusIcon %s: "
          "delta: %d , direction: %s , time: %u",
          gtk_status_icon_get_title(status_icon), delta,
          direction_to_str(direction), _time);

    g_signal_emit(icon, signals[SCROLL], 0, delta, x_dir, _time);
  }

  return GDK_EVENT_PROPAGATE;
}

static void name_owner_changed(GDBusConnection *connection,
...
static void add_name_listener(XAppStatusIcon *self) {
  DEBUG("Adding NameOwnerChanged listener for status monitors");

  self->priv->listener_id = g_dbus_connection_signal_subscribe(
      self->priv->connection, FDO_DBUS_NAME, FDO_DBUS_NAME, "NameOwnerChanged",
      FDO_DBUS_PATH, STATUS_ICON_MONITOR_MATCH,
      G_DBUS_SIGNAL_FLAGS_MATCH_ARG0_NAMESPACE, name_owner_changed, self, NULL);
}

static void on_name_lost(GDBusConnection *connection, const gchar *name,
                         gpointer user_data) {
  g_warning("XAppStatusIcon: lost or could not acquire presence on dbus.  "
            "Refreshing.");

  GList *instances =
      g_dbus_object_manager_get_objects(G_DBUS_OBJECT_MANAGER(obj_server));
  GList *l;

  for (l = instances; l != NULL; l = l->next) {
    GObject *instance = G_OBJECT(l->data);
    XAppStatusIcon *icon = XAPP_STATUS_ICON(
        g_object_get_data(instance, "xapp-status-icon-instance"));

    if (icon == NULL) {
      g_warning(
          "on_name_lost: Could not retrieve xapp-status-icon-instance data: %s",
          name);
      continue;
    }
...
static void sync_skeleton(XAppStatusIcon *self) {
  ...
  g_object_set(G_OBJECT(priv->interface_skeleton), "name", priv->name, "label",
               priv->label, "icon-name", priv->icon_name, "tooltip-text",
               priv->tooltip_text, "visible", priv->visible, "metadata",
               priv->metadata, NULL);

  g_dbus_interface_skeleton_flush(
      G_DBUS_INTERFACE_SKELETON(priv->interface_skeleton));
}
...
static void on_name_acquired(GDBusConnection *connection, const gchar *name,
  ...
  for (l = instances; l != NULL; l = l->next) {
    ...
    if (icon == NULL) {
      g_warning("on_name_aquired: Could not retrieve xapp-status-icon-instance "
                "data: %s",
                name);
      continue;
    }
...
static void ensure_object_manager(XAppStatusIcon *self) {
  if (obj_server == NULL) {
    ...
    g_dbus_object_manager_server_set_connection(obj_server,
                                                self->priv->connection);
    g_object_weak_ref(G_OBJECT(obj_server), (GWeakNotify)obj_server_finalized,
                      self);
  } else {
...
static gboolean export_icon_interface(XAppStatusIcon *self) {
  gint i;
  ...
  xapp_object_skeleton_set_status_icon_interface(
      self->priv->object_skeleton, self->priv->interface_skeleton);

  g_object_set_data(G_OBJECT(self->priv->object_skeleton),
                    "xapp-status-icon-instance", self);

  g_dbus_object_manager_server_export_uniquely(
      obj_server, G_DBUS_OBJECT_SKELETON(self->priv->object_skeleton));
  ...
  for (i = 0; i < G_N_ELEMENTS(skeleton_signals); i++) {
    ...
    g_signal_connect(self->priv->interface_skeleton, sig.signal_name,
                     G_CALLBACK(sig.callback), self);
  }

  return TRUE;
}

static void connect_with_status_applet(XAppStatusIcon *self) {
  ...
  if (g_dbus_is_name(self->priv->name) &&
    ...
    gchar *valid_app_name =
        g_strdelimit(g_strdup(g_get_prgname()), " .-,=+~`/", '_');

    owner_name = g_strdup_printf("%s.%s", ICON_NAME, valid_app_name);
  ...
  if (name_owner_id == 0) {
    ...
    name_owner_id = g_bus_own_name_on_connection(
        self->priv->connection, owner_name, G_BUS_NAME_OWNER_FLAGS_DO_NOT_QUEUE,
...
static void update_fallback_icon(XAppStatusIcon *self) {
  ...
  if (priv->icon_name) {
    ...
    if (g_path_is_absolute(priv->icon_name)) {
      ...
      gtk_status_icon_set_from_icon_name(priv->gtk_status_icon,
                                         priv->icon_name);
    }
  } else {
...
static void on_gtk_status_icon_embedded_changed(GtkStatusIcon *icon,
                                                GParamSpec *pspec,
                                                gpointer user_data) {
...
static void use_gtk_status_icon(XAppStatusIcon *self) {
  ...
  g_signal_connect(priv->gtk_status_icon, "button-press-event",
                   G_CALLBACK(on_gtk_status_icon_button_press), self);
  g_signal_connect(priv->gtk_status_icon, "button-release-event",
                   G_CALLBACK(on_gtk_status_icon_button_release), self);
  g_signal_connect(priv->gtk_status_icon, "scroll-event",
                   G_CALLBACK(on_gtk_status_icon_scroll), self);
  g_signal_connect(priv->gtk_status_icon, "notify::embedded",
                   G_CALLBACK(on_gtk_status_icon_embedded_changed), self);

  update_fallback_icon(self);
}

static void on_list_names_completed(GObject *source, GAsyncResult *res,
                                    gpointer user_data) {
  XAppStatusIcon *self = XAPP_STATUS_ICON(user_data);
  GVariant *result;
  GVariantIter *iter;
...
static void look_for_status_applet(XAppStatusIcon *self) {
  ...
  g_dbus_connection_call(
      self->priv->connection, FDO_DBUS_NAME, FDO_DBUS_PATH, FDO_DBUS_NAME,
      "ListNames", NULL, G_VARIANT_TYPE("(as)"), G_DBUS_CALL_FLAGS_NONE,
      3000, /* 3 secs */
      self->priv->cancellable, on_list_names_completed, self);
}

static void complete_icon_setup(XAppStatusIcon *self) {
...
static void on_session_bus_connected(GObject *source, GAsyncResult *res,
                                     gpointer user_data) {
  XAppStatusIcon *self = XAPP_STATUS_ICON(user_data);
  GError *error;
  ...
  if (error != NULL) {
    if (error->code != G_IO_ERROR_CANCELLED) {
      g_critical("XAppStatusIcon: Unable to acquire session bus: %s",
                 error->message);

      /* If we never get a connection, we use the Gtk icon exclusively, and will
       * never re-try.  FIXME? this is unlikely to happen, so I don't see the
       * point in trying later, as there are probably bigger problems in this
       * case. */
      use_gtk_status_icon(self);
    } else {
...
static void refresh_icon(XAppStatusIcon *self) {
  if (self->priv->connection == NULL) {
    ...
    g_bus_get(G_BUS_TYPE_SESSION, self->priv->cancellable,
              on_session_bus_connected, self);
  } else {
    complete_icon_setup(self);
  }
}

static void xapp_status_icon_set_property(GObject *object, guint prop_id,
                                          const GValue *value,
                                          GParamSpec *pspec) {
  switch (prop_id) {
  case PROP_PRIMARY_MENU:
    xapp_status_icon_set_primary_menu(XAPP_STATUS_ICON(object),
                                      g_value_get_object(value));
    break;
  case PROP_SECONDARY_MENU:
    xapp_status_icon_set_secondary_menu(XAPP_STATUS_ICON(object),
                                        g_value_get_object(value));
    break;
  case PROP_ICON_SIZE:
    XAPP_STATUS_ICON(object)->priv->icon_size =
        CLAMP(g_value_get_int(value), 0, MAX_SANE_ICON_SIZE);
    break;
  case PROP_NAME: {
    ...
    xapp_status_icon_set_name(XAPP_STATUS_ICON(object),
...
static void xapp_status_icon_get_property(GObject *object, guint prop_id,
...
static void xapp_status_icon_init(XAppStatusIcon *self) {
...
static void remove_icon_path_from_bus(XAppStatusIcon *self) {
  ...
  if (self->priv->object_skeleton) {
    const gchar *path;
    path = g_dbus_object_get_object_path(
        G_DBUS_OBJECT(self->priv->object_skeleton));

    DEBUG("Removing interface at path '%s'", path);

    g_object_set_data(G_OBJECT(self->priv->object_skeleton),
                      "xapp-status-icon-instance", NULL);
...
static void xapp_status_icon_dispose(GObject *object) {
  ...
  g_clear_pointer(&self->priv->tooltip_text, g_free);
  g_clear_pointer(&self->priv->label, g_free);
  g_clear_pointer(&self->priv->metadata, g_free);
  ...
  if (self->priv->gtk_status_icon != NULL) {
    g_signal_handlers_disconnect_by_func(self->priv->gtk_status_icon,
                                         on_gtk_status_icon_button_press, self);
    g_signal_handlers_disconnect_by_func(
        self->priv->gtk_status_icon, on_gtk_status_icon_button_release, self);
...
static void xapp_status_icon_finalize(GObject *object) {
  DEBUG("XAppStatusIcon finalize (%p)", object);

  G_OBJECT_CLASS(xapp_status_icon_parent_class)->finalize(object);
}

static void xapp_status_icon_class_init(XAppStatusIconClass *klass) {
   ...
   *
   * A #GtkMenu to use when requested by the remote monitor via a left (or
   * primary) click.
   *
   * When this property is not %NULL, the menu will be automatically positioned
   * and displayed during a primary button release.
   *
   * When this property IS %NULL, the #XAppStatusIcon::activate will be sent for
   * primary button presses.
   *
   * In both cases, the #XAppStatusIcon::button-press-event and
   * #XAppStatusIcon::button-release-events will be fired like normal.
   *
   * Setting this will remove any floating reference to the menu and assume
   * ownership. As a result, it is not necessary to maintain a reference to it
   * in the parent application (or unref it when finished with it - if you wish
   * to replace the menu, simply call this method again with a new menu.
   *
   * The same #GtkMenu widget can be set as both the primary and secondary.
   */
  g_object_class_install_property(
      gobject_class, PROP_PRIMARY_MENU,
      g_param_spec_object(
          "primary-menu", "Status icon primary (left-click) menu",
   ...
   *
   * A #GtkMenu to use when requested by the remote monitor via a right (or
   * secondary) click.
   *
   * When this property is not %NULL, the menu will be automatically positioned
   * and displayed during a secondary button release.
   *
   * When this property IS %NULL, the #XAppStatusIcon::activate will be sent for
   ...
   */
  g_object_class_install_property(
      gobject_class, PROP_SECONDARY_MENU,
      g_param_spec_object(
          "secondary-menu", "Status icon secondary (right-click) menu",
   ...
   *
   * The icon size that is preferred by icon monitor/host - this is usually a
   * product of some calculation based on the panel size.  It can be used by the
   * client to size an icon to be saved as a file and its path sent to the host.
   *
   * If this value is 0 it has not been set, and its value can be unreliable if
   * the host has multiple #XAppStatusIconMonitors active.
   */
  g_object_class_install_property(
      gobject_class, PROP_ICON_SIZE,
      g_param_spec_int("icon-size", "The icon size the monitor/host prefers",
                       "The icon size that should be used, if the client is"
                       " supplying absolute icon paths",
                       0, 96, 0, G_PARAM_READWRITE));

  /**
   * XAppStatusIcon:name:
   *
   * The name of the icon for sorting purposes. If this is in the form of
   * 'org.x.StatusIcon.foo` and set immediately upon creation of the icon, it
   * will also attempt to own this dbus name; this can be useful in sandboxed
   * environments where a well-defined name is required. If additional icons are
   * created, only the name given to the initial one will be used for dbus,
   * though different names can still affect the sort order. This is set to the
   * value of g_get_prgname() if no other name is provided.
   */
  g_object_class_install_property(
      gobject_class, PROP_NAME,
      g_param_spec_string("name", "The name of the icon for sorting purposes.",
                          NULL, NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
   ...
   */
  signals[BUTTON_PRESS] = g_signal_new(
      "button-press-event", XAPP_TYPE_STATUS_ICON,
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, 0, NULL, NULL, NULL, G_TYPE_NONE, 5,
      G_TYPE_INT, G_TYPE_INT, G_TYPE_UINT, G_TYPE_UINT, G_TYPE_INT);
   ...
   */
  signals[BUTTON_RELEASE] = g_signal_new(
      "button-release-event", XAPP_TYPE_STATUS_ICON,
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, 0, NULL, NULL, NULL, G_TYPE_NONE, 5,
      G_TYPE_INT, G_TYPE_INT, G_TYPE_UINT, G_TYPE_UINT, G_TYPE_INT);
   ...
   * XAppStatusIcon:primary-menu or XAppStatusIcon:secondary-menu is not %NULL,
   * this signal is skipped for the respective button presses.  A middle button
   * click will always send this signal when pressed.
   */
  signals[ACTIVATE] = g_signal_new(
      "activate", XAPP_TYPE_STATUS_ICON, G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION,
  ...
  signals[STATE_CHANGED] =
      g_signal_new("state-changed", XAPP_TYPE_STATUS_ICON,
   ...
   * icon. For the most part, amounts will always be 1, unless an applet
   * supports smooth scrolling.  Generally the direction value is most
   * important.
   */
  signals[SCROLL] = g_signal_new("scroll-event", XAPP_TYPE_STATUS_ICON,
                                 G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION, 0, NULL,
                                 NULL, NULL, G_TYPE_NONE, 3, G_TYPE_INT,
                                 XAPP_TYPE_SCROLL_DIRECTION, G_TYPE_UINT);
}
...
void xapp_status_icon_set_icon_name(XAppStatusIcon *icon,
  ...
  if (icon->priv->interface_skeleton) {
    xapp_status_icon_interface_set_icon_name(icon->priv->interface_skeleton,
                                             icon_name);
  }
...
gint xapp_status_icon_get_icon_size(XAppStatusIcon *icon) {
  ...
  size =
      xapp_status_icon_interface_get_icon_size(icon->priv->interface_skeleton);
...
void xapp_status_icon_set_tooltip_text(XAppStatusIcon *icon,
  ...
  if (icon->priv->interface_skeleton) {
    xapp_status_icon_interface_set_tooltip_text(icon->priv->interface_skeleton,
                                                tooltip_text);
  }
...
void xapp_status_icon_set_visible(XAppStatusIcon *icon,
  ...
  if (icon->priv->interface_skeleton) {
    xapp_status_icon_interface_set_visible(icon->priv->interface_skeleton,
                                           visible);
  }
 ...
 * @icon: an #XAppStatusIcon
 * @menu: (nullable): A #GtkMenu to display when the primary mouse button is
 * released.
 ...
 * Since: 1.8.6
 */
void xapp_status_icon_popup_menu(XAppStatusIcon *icon, GtkMenu *menu, gint x,
                                 gint y, guint button, guint _time,
                                 gint panel_position)

{
  ...
  popup_menu(icon, menu, x, y, button, _time, panel_position);
}
 ...
 *
 * Returns: (transfer full): a new #XAppStatusIcon. Use g_object_unref when
 * finished.
 *
 * Since: 1.6
 */
...
XAppStatusIcon *xapp_status_icon_new_with_name(const gchar *name) {
  return g_object_new(XAPP_TYPE_STATUS_ICON, "name", name, NULL);
}
 ...
 * @icon: an #XAppStatusIcon
 *
 * Gets the current #XAppStatusIconState of icon. The state is determined by
 * whether the icon is being displayed by an #XAppStatusMonitor client, a
 * fallback tray icon, or not being displayed at all.
 *
...
XAppStatusIconState xapp_status_icon_get_state(XAppStatusIcon *icon) {
  g_return_val_if_fail(XAPP_IS_STATUS_ICON(icon),
 ...
 *
 * Sets metadata to pass to the icon proxy for an applet's use. Right now this
 * is only so xapp-sn-watcher can tell the applets when the icon is originating
 * from appindicator so panel button 'highlighting' can behave correctly.
 *
 * Since: 1.8.7
 */
void xapp_status_icon_set_metadata(XAppStatusIcon *icon,
  ...
  if (icon->priv->interface_skeleton) {
    xapp_status_icon_interface_set_metadata(icon->priv->interface_skeleton,
                                            metadata);
  }
}
...
gboolean xapp_status_icon_any_monitors(void) {
  ...
  if (connection) {
    ...
    result = g_dbus_connection_call_sync(
        connection, FDO_DBUS_NAME, FDO_DBUS_PATH, FDO_DBUS_NAME, "ListNames",
        NULL, G_VARIANT_TYPE("(as)"), G_DBUS_CALL_FLAGS_NONE,
        /* 2 seconds */ 2 * 1000, NULL, &error);
